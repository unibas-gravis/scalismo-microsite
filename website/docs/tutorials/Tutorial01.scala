object Tutorial1 extends App {
  ...
  val mesh: TriangleMesh[_3D] =
    MeshIO.readMesh(new java.io.File("datasets/Paola.ply")).get
  ...
  val vectorSum = vectors.reduce { (v1, v2) =>
    v1 + v2
  ...
  val image: DiscreteImage[_3D, Short] = ImageIO
    .read3DScalarImage[Short](new java.io.File("datasets/PaolaMRI.vtk"))
    .get
  ...
  val gridPointsView =
    ui.show(paolaGroup, imagePoints.toIndexedSeq, "imagePoints")
  val values: Iterator[Short] = image.values
  image.values.next
  image(IntVector(0, 0, 0))
  image.values.size == image.domain.pointSet.numberOfPoints
  val threshValues = image.values.map { v: Short =>
    if (v <= 300) v else 0.toShort
  }
  ...
  val faceModel: PointDistributionModel[_3D, TriangleMesh] = StatisticalModelIO
    .readStatisticalTriangleMeshModel3D(new java.io.File("datasets/bfm.h5"))
    .get
  ...
  val matchingLandmarks: Seq[Landmark[_3D]] =
    matchingLandmarkViews.map(lmView => lmView.landmark)
