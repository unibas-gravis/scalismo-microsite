...
object Tutorial0 extends App {
  ...
  val alignedMeshes = toAlign.map { mesh =>
    val landmarks = pointIds.map { id =>
      Landmark("L_" + id, mesh.pointSet.point(PointId(id)))
    }
  ...
  val continuousFields =
    defFields.map(f => f.interpolate(TriangleMeshInterpolator3D()))
  ...
  val modelFromDataCollection: PointDistributionModel[_3D, TriangleMesh] =
  ...
  val modelFromDataCollectionGPA =
    PointDistributionModel.createUsingPCA(dcWithGPAAlignedShapes)
