...
object Tutorial14 extends App {
  ...
  case class LikelihoodEvaluator(data: Seq[(Double, Double)])
    ...
    override def logValue(theta: Sample): Double = {
      ...
      val likelihoods = for ((x, y) <- data) yield {
        val likelihood = breeze.stats.distributions
          .Gaussian(theta.parameters.a * x + theta.parameters.b, theta.parameters.sigma2)
  ...
  case class RandomWalkProposal(stepLengthA: Double, stepLengthB: Double, stepLengthSigma2: Double)(
    ...
    override def logTransitionProbability(from: Sample, to: Sample): Double = {
      ...
      stepDistA.logPdf(residualA) + stepDistB.logPdf(residualB) + stepDistSigma2.logPdf(
        residualSigma2
      )
    }
  }
